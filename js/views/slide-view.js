...
export class Slide {
  ...
  updatePosition(clientX) {
    this.dist.movement = (this.dist.startX - clientX) * DRAG_SENSITIVITY;
  ...
  changeSlideOnEnd() {
    if (this.dist.movement > DRAG_THRESHOLD && this.index.next !== undefined) {
      this.activeNextSlide();
    } else if (this.dist.movement < -DRAG_THRESHOLD && this.index.prev !== undefined) {
  ...
  addSlideClickEvents() {
    this.slideArray.forEach((item, index) => {
      item.element.addEventListener('click', () => this.changeSlide(index));
    });
  }
...
export default class SlideNav extends Slide {
  ...
  createControl() {
    ...
    this.slideArray.forEach((item, index) => {
      ...
      controlLink.href = `#slide${index + 1}`;
      controlLink.textContent = index + 1;
